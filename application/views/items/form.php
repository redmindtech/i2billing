...
<?php echo form_open('items/save/'.$item_info->item_id, array('id'=>'item_form', 'enctype'=>'multipart/form-data', 'class'=>'form-horizontal')); ?>
	<fieldset id="item_basic_info">
		...
		<div class="form-group form-group-sm">
			...
			<div class="col-xs-4">
				<div class="input-group input-group-sm">
					...
					<?php endif; ?>
					<?php echo form_input(array( 'min'=>1,
							'name'=>'cost_price',
							'id'=>'cost_price',
							'class'=>'form-control input-sm',
							'onClick'=>'this.select();',
		...
		?>
			<div class="form-group form-group-sm">
				<?php echo form_label($this->lang->line('items_quantity').' '.$location_detail['location_name'], 'quantity_' . $key, array('class'=>'control-label col-xs-3')); ?>
				<div class='col-xs-4'>
					<?php echo form_input(array(
							'name'=>'quantity_' . $key,
							'id'=>'quantity_' . $key,
	...
	var init_validation = function() {
		$('#item_form').validate($.extend({
			...
			rules:
			{
				...
				{
					required: true,
					remote: "<?php echo site_url($controller_name . '/cost_price_validate')?>"
				},
				unit_price:
				{
					required: true,
					remote: "<?php echo site_url($controller_name . '/sale_price_validate')?>"
				},
				<?php
				foreach($stock_locations as $key=>$location_detail)
				{
